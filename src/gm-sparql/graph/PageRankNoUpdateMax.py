...
class PageRankNoUpdateMax(GraphAlgorithmModule.GraphAlgorithm):
    ...
    def __init__(self):
        GraphAlgorithmModule.GraphAlgorithm.__init__(self, 'PageRankNoUpdateMax')
    ...
    def initialize(self, dampingFactor, limitIteration, directed):
        ...
        results = self.connection.urika.query(self.name, PageRankNoUpdateMax.COMMAND_TO_RETRIEVE_NUMBER_OF_VERTICES, None, None, 'json', True) 
        ...
        print self.convergenceThreshold
        self.connection.urika.update(self.name, PageRankNoUpdateMax.COMMAND_TO_INITIALIZE_WORKING_GRAPH)
        if directed == True:
            self.connection.urika.update(self.name, PageRankNoUpdateMax.COMMAND_TO_STORE_OUT_DEGREES)
        else:
            self.connection.urika.update(self.name, PageRankNoUpdateMax.COMMAND_TO_STORE_OUT_DEGREES_UNDIRECTED)

        self.connection.urika.update(self.name, PageRankNoUpdateMax.COMMAND_TO_STORE_INITIAL_PAGERANKS.format(1.0 / float(self.numberOfVertices)))

    def process(self, directed):
        for iteration in range(0, self.limitIteration):
            start = time.time()
            if directed == True:
                self.connection.urika.update(self.name, PageRankNoUpdateMax.COMMAND_TO_UPDATE_PAGERANKS.format(DAMPING_FACTOR = self.dampingFactor, STAYING_FACTOR = ((1.0 - self.dampingFactor) / float(self.numberOfVertices)), THRESHOLD = self.convergenceThreshold))
            else:
                self.connection.urika.update(self.name, PageRankNoUpdateMax.COMMAND_TO_UPDATE_PAGERANKS_UNDIRECTED.format(DAMPING_FACTOR = self.dampingFactor, STAYING_FACTOR = ((1.0 - self.dampingFactor) / float(self.numberOfVertices)), THRESHOLD = self.convergenceThreshold))
            end = time.time()
    ...
    def finalize(self):
        ...
        results = self.connection.urika.query(self.name, PageRankNoUpdateMax.COMMAND_TO_GATHER_RESULT, None, None, 'json', True)
        for result in results['results']['bindings']:
            writer.writeLine('Vertex: ' + result['vertex']['value'] + ', PageRank: ' + result['pagerank']['value'])

        writer.close()
        self.connection.urika.update(self.name, PageRankNoUpdateMax.COMMAND_TO_FINALIZE_WORKING_GRAPH)
    
    def isConverged(self):
        start = time.time()
        results = self.connection.urika.query(self.name, PageRankNoUpdateMax.COMMAND_TO_CHECK_DIFFERENCE, None, None, 'json', True)
        end = time.time()
        print 'Convergence Check: ' + str(end - start) + ' seconds'
        difference = int(results['results']['bindings'][0]['exist']['value'])
        return difference == 0
