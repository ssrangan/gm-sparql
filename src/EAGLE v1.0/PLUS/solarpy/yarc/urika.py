...
# These parameters are now set after login TFY, ORNL 7/23/2014
    ...
    def query( self, label, string = None, file = None, block= None, accept= "json", verbose= True, big_results = False, **params): #added oftype by TFY, ORNL to be able to query with strings and not just .rq files
        ...
        try:
            if big_results:
                response = self.query_get( label, text, acceptTypes.get( accept ), big_results=True, **params )
            else:
