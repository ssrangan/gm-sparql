...
class initsolarpy ():
    ...
    def __init__ (self, usern, passw, db, host, flavor):
        import logging
        import traceback
        import optparse
        ...
        if flavor == "fuseki":
            self.options.url = "%s%s/" %(self.options.url, db)
...
def UrikaLogin(usern, passw, db = None, method = "solarpy", host = "https://apollo.ccs.ornl.gov/"):
    if _urikaCheck():
        if method == "solarpy":
            solarpy = initsolarpy(usern, passw, db, host, flavor = "urika" )
            return(solarpy)

        elif method == "sparqlwrapper":
            wrapper = initwrapper(usern, passw, db, host, flavor = "urika")
            return(wrapper)
    else:
        exit()

def FusekiLogin(db, method = "solarpy", host = "http://localhost:3030/"):
    if method == "solarpy":
        solarpy = initsolarpy(None, None, db, host, flavor = "fuseki")
        return(solarpy)
    elif method == "sparqlwrapper":
        wrapper = initwrapper(None, None, db, host, flavor = "fuseki")
        return(wrapper)
